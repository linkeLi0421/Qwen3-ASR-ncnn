...
## 14. 从 FunASR / Qwen3-ASR 侧补充的验证建议

如果后续有同学继续做 ncnn 移植，建议把“ncnn 输出最终文本须与 PyTorch 原版一致”
拆成几层测，否则很容易只测到 demo 音频能跑。

第一层是固定音频前处理。需要明确 sample rate、mono 转换、padding/truncation、
mel/filterbank 参数、chunk window 和 overlap 参数。建议同时保存 PyTorch 路径和
ncnn 路径的 encoder 输入摘要，例如 shape、dtype、min/max/mean/std，以及前几个
frame 或 bin 的统计量。这样可以避免差异其实来自音频前处理，而不是模型转换。

第二层是端到端文本对齐。建议至少准备三类 fixture：短中文普通话、较长中文音频、
中文夹英文/数字/标点的音频。比较时先比较最终 normalized text；如果模型输出带
timestamp、特殊 token 或额外标记，再单独比较清洗前文本和清洗后文本，避免把
postprocess 问题误判成推理问题。

第三层是模块级定位。Qwen3-ASR 这类结构通常可以按 speech encoder、projector /
adaptor、Qwen decoder 三段定位。转换时最好分别记录 encoder embedding、
projected embedding、首轮 decoder logits 的误差范围，例如 `max_abs`、`mean_abs`
和 `p99_abs`。这样排查 pnnx/ncnn 算子误差会快很多。

第四层是多平台 smoke。Linux + Windows 编译运行之外，建议至少记录 CPU 型号和
指令集、线程数、RTF/耗时、峰值内存、是否启用 Vulkan/GPU。如果后续移动端走
Vulkan/ARM，建议单独补性能表，不要把移动端性能结论和 PyTorch accuracy parity
混在一个结论里。
...
如果后续有公开 repo 或 PR，我可以从 FunASR / Qwen3-ASR 兼容性角度帮忙看测试覆盖
和输出契约。
