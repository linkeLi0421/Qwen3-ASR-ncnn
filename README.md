...
最新实现分支已补 raw tensor parity 入口：`qwen3_asr_main --dump-module-raw DIR`
和 `run_pytorch_fixtures.py --dump-module-raw` 会写出双方 `.f32` tensor；
`evaluate_fixtures.py` 在双方都有 `raw_path` 时会计算 `max_abs`、`mean_abs`、
`p99_abs`、cosine，以及 selected logits 的 top-5 agreement。真实三条 fixture 的
raw 数值还需要在 VM 上重新生成。
...
1. 在 VM 上用 `--dump-module-raw` 重跑三条 long-window fixture，生成真实 raw tensor 误差表。
